...
export default function StatLeadersMini() {
    ...
    useEffect(() => {
        ...
        fetchSheet(BATTING_URL, setBatting);
        fetchSheet(PITCHING_URL, setPitching);
    }, []);
        ...
        return [...data]
            .filter(row => {
                ...
                if (stat === "ERA") {
                    ...
                    if (games < 1 || innings < 10) return false;
                }
    ...
    const pages = [
        { title: "Hits", stat: "H", data: batting },
        { title: "Home Runs", stat: "HR", data: batting },
    ...
    const topPlayers = getTopPlayers(
        currentPage.data,
        currentPage.stat,
        currentPage.isLowerBetter
    );

    const getPlayerName = (row: StatRow) =>
        row["Player"] || row[Object.keys(row)[0]] || "Unknown";

    return (
        <LeadersWrapper>
            <Category>
                <div style={{display: "flex", alignItems: "center", justifyContent: "center", marginTop: "-1rem", marginBottom: "1.5rem",}}>
                    <ArrowButton onClick={() => setPage(prev => (prev - 1 + pages.length) % pages.length)}>◀</ArrowButton>

                    <CategoryTitle style={{ fontSize: "2rem", margin: 0 }}>{currentPage.title}</CategoryTitle>
                ...
                {topPlayers.map((player, i) => (
                    <PlayerRow key={i}>
                        <div style={{ display: "flex", gap: "0.25rem", minWidth: 0 }}>
                            ...
                            <span style={{overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",}}>{getPlayerName(player)}</span>
                        </div>
